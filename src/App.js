...
import Models, { ModelMatrix } from './Models'
...
function Scene(props) {
  ...
  const scene_options = useMemo(() => {
    return {
      ...
      visible: true,
      color: { value: 'lime' },
      fool: button((get) => console.log(modelref.current.matrixWorld.elements))
    }
  }, [])
  ...
  return (
    <group {...props} dispose={null}>
      <PivotControls ref={modelref} rotation={[0, -Math.PI / 2, 0]} anchor={[1, -1, -1]} scale={75} depthTest={false} fixed lineWidth={2}>
...
function Env({ perfSucks }) {
  ...
  const light_opt1 = useMemo(() => {
    return {
      ...
      foo: button((get) => console.log(ModelData()[7].current.matrixWorld.elements)),
      visible: true,
      color: { value: 'lime' }
    }
  }, [])
  ...
  for (let i = 0; i < ModelData().length; i++) {
    if (ModelData()[i] && ModelData()[i].current) modeljson.push(new ModelMatrix(i, ModelData()[i].current.matrixWorld.elements, null))
  }
  console.log(modeljson)
  useFrame((state, delta) => {
    // console.log(cakeref.position)
