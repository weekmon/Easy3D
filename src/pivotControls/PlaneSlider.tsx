...
const decomposeIntoBasis = (e1: THREE.Vector3, e2: THREE.Vector3, offset: THREE.Vector3) => {
  const i1 =
    Math.abs(e1.x) >= Math.abs(e1.y) && Math.abs(e1.x) >= Math.abs(e1.z) ? 0 : Math.abs(e1.y) >= Math.abs(e1.x) && Math.abs(e1.y) >= Math.abs(e1.z) ? 1 : 2
    ...
    <group ref={objRef} matrix={matrixL} matrixAutoUpdate={false}>
      ...
      <group position={[pos1 * 1.7, pos1 * 1.7, 0]}>
        <mesh
          ...
          userData={userData}>
          <planeGeometry />
          <meshBasicMaterial transparent depthTest={depthTest} color={color} polygonOffset polygonOffsetFactor={-10} side={THREE.DoubleSide} />
        </mesh>
        <Line
